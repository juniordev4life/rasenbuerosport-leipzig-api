 ...
 *
 * Runs the two-stage team pipeline in order:
 ...
 *      (ON CONFLICT (name) DO UPDATE, preserving existing UUIDs)
 *
 * DATABASE_URL is required and checked up front, so a missing connection
 * string fails fast — before any parsing work is done.
 *
 * Run:
 ...
 * @returns {void}
 * @example
 * runStep("parse-sofifa-leagues.js", "Step 1/2 — Parse SoFIFA leagues");
 */
...
console.log("Updating teams: parse → import");

try {
	runStep("parse-sofifa-leagues.js", "Step 1/2 — Parse SoFIFA leagues");
	runStep("import-teams.js", "Step 2/2 — Upsert teams into Cloud SQL");
	console.log("\n✓ Teams update complete.");
} catch (err) {
